...
fun <T : Any?, R : Any?> LiveData<T>.map(mapper: (T?) -> R?): LiveData<R> =
    MediatorLiveData<R>().also { mediator ->
        mediator.addSource(this) { mediator.value = mapper(it) }
    }

/**
 * Will ignore null values returned from [mapper]
 */
fun <T : Any?, R : Any?> LiveData<T>.mapNotNull(mapper: (T?) -> R?): LiveData<R> =
    map(mapper).filterNotNull()


fun <T1 : Any?, T2 : Any?, R> LiveData<T1>.zip(with: LiveData<T2>, zipper: (T1?, T2?) -> R): LiveData<R> =
    MediatorLiveData<R>().also { mediator ->
